...
var cart_router_exports = {};
__export(cart_router_exports, {
  cartRouter: () => cartRouter
});
module.exports = __toCommonJS(cart_router_exports);
...
async function getCart3(req, res) {
  ...
  try {
    ...
    return res.status(import_http_status.default.OK).send(cart);
  } catch (error) {
...
async function getCarts3(req, res) {
  try {
    ...
    return res.status(import_http_status.default.OK).send(carts);
  } catch (error) {
    return res.status(import_http_status.default.NOT_FOUND).send(error.message);
  }
}
async function createCart3(req, res) {
  ...
  try {
    ...
    return res.status(import_http_status.default.CREATED).send(cart);
  } catch (error) {
...
// src/routers/cart-router.ts
var cartRouter = (0, import_express.Router)();
